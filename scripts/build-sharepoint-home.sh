...
# "All docs" folder-browse link (the one place a folder view is intended).
ALL_DOCS="${SITE_URL}/Shared%20Documents/Forms/AllItems.aspx?id=%2Fsites%2Fit-project-docs%2FShared%20Documents%2Fwebfront%2Fdocs"
...
#
# Remove section 1 repeatedly until the call errors (you can't remove the
# last/only section, so exactly one empty default section always remains).
...
#   1  intro                       (one column)
#   2  onboarding documentation    (one column)
#   3  read the docs | behind it   (two columns)
#   4  Community app | API | DevOps (three columns)
#   5  Architecture | Hosting | M365 (three columns)
#   6  app downloads: desktop, then mobile (one column, two stacked blocks)
#   7  how this page is published  (one column)
#
# IMPORTANT — add ALL section shells FIRST, then fill text. If section+text
# are interleaved, m365 collapses the first two single-column sections into one
# zone (intro + onboarding end up stacked together with onboarding on top).
# Creating every shell up front establishes the zones cleanly so each text add
# lands in its own section.

say "adding section shells (1 intro · 2 onboarding · 3 links · 4-5 cards · 6 downloads · 7 publish-info)…"
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate OneColumn   --order 1 >/dev/null
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate OneColumn   --order 2 >/dev/null
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate TwoColumn   --order 3 >/dev/null
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate ThreeColumn --order 4 >/dev/null
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate ThreeColumn --order 5 >/dev/null
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate OneColumn   --order 6 >/dev/null
m365 spo page section add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" --sectionTemplate OneColumn   --order 7 >/dev/null
ok "7 section shells added"
...
ok "app downloads added"

# --- section 7: how this page is published ------------------------------------
say "section 7 — how this page is published…"
m365 spo page text add --pageName "$PAGE_NAME" --webUrl "$SITE_URL" \
