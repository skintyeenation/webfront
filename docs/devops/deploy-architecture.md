...
| [`scripts/setup-lookup-azure.sh`](../../scripts/setup-lookup-azure.sh) | `lookup-prod` Container App (min 1 replica) + (optional) ANTHROPIC_API_KEY secret + LOOKUP_CONTAINERAPP variable + `deploy-lookup` pipeline | `setup-api-azure.sh` |
| [`scripts/set-lookup-api-key.sh`](../../scripts/set-lookup-api-key.sh) | Helper — set/rotate the `ANTHROPIC_API_KEY` Container App secret on `lookup-prod` independent of the setup. Idempotent. | `setup-lookup-azure.sh` |
