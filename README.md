...
- [`docs/devops/azure-backup.md`](docs/devops/azure-backup.md) — planning doc for the Azure **resource config** backup (workload 4 of 4): ARM/Bicep templates, RBAC, alert rules, Container App config, Postgres server config — config snapshots in version-controlled JSON (Postgres data + Blob contents are intentionally separate concerns)
