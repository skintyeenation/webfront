...
| &nbsp;&nbsp;&nbsp;&nbsp;[Password policy (Entra)](docs/365/password-policy.md) | Entra cloud + on-prem AD password rules — complexity, **never-expire**, smart lockout (live values) |
...
| **Vaultwarden** (shared password manager, ADR-18) — vault contents in the `vaultwarden` Postgres db **+** the `/data` Azure Files share (RSA JWT key, attachments, Sends, `config.json`) | `vaultwarden-backup` | [`vaultwarden/README.md` § Backup](vaultwarden/README.md#backup) | ⬜ Planning — db via `pg_dump` (extends `postgres-dumps`; Microsoft 7-day PITR is the near-term floor) + Azure Files snapshot/soft-delete → Blob copy. **Not implemented yet.** |
...
| **Secret VALUES** (Postgres password, SP client secrets, SAS tokens, Anthropic API key) | These are NOT files that need backing up — they live in **Vaultwarden** (the durable source of truth, ADR-18 — was 1Password) + ADO variable group + Container App secrets (the operational copies). The backup snapshots record that a secret *exists*, never the value itself. | Vaultwarden — IT/Admin collection (`vault.skintyee.ca`) |
...
- [`docs/hardcoded-values.md`](docs/hardcoded-values.md) — known sites of duplication where the same GUID / slug / mail address / display name is hardcoded in more than one file because there's no shared module yet. Each entry lists the canonical source, every duplicate site, the drift symptom (what breaks when they disagree), and the fix path to consolidate. Shrinks as the codebase consolidates.
...
- [`docs/features/member-provisioning.md`](docs/features/member-provisioning.md) — **Planned (ADR-15)** — turns the Add Member admin form from a Redux-only stub into a real Entra-backed provisioning flow: creates the M365 user via Microsoft Graph (`POST /users`), assigns an available license via `POST /users/{id}/assignLicense`, seats the user into the picked Entra security groups via `POST /groups/{id}/members/$ref`, optionally chains a `Person` create with timesheets enabled. Four slices + a shared `appRole` derivation helper. Three new application permissions on `skintyee-app-graph` (`User.ReadWrite.All`, `LicenseAssignment.ReadWrite.All`, `Organization.Read.All`). No PowerShell layer — Graph covers all three operations.
...
- [`docs/365/device-identity-vs-management.md`](docs/365/device-identity-vs-management.md) — **device identity vs. management vs. compliance** — why Hybrid joined ≠ Intune managed, the `isCompliant` tri-state (`null` = "no policy", not red), duplicate device registrations (the stale `Workplace` object left after Hybrid Join) + how the app/API consolidate by computer name; verify with `Verify-HybridJoin.ps1`
