...
| [`Enable-PasswordWritebackPermissions.ps1`](Enable-PasswordWritebackPermissions.ps1) | SSPR | Grants the `MSOL_*` connector account *Reset/Change Password* + write `lockoutTime`/`pwdLastSet` on descendant user objects of `OU=SkinTyee Users` — the AD-permissions piece of SSPR password writeback. **Preview by default; `-Apply` to write.** Then run `Set-ADSyncAADPasswordResetConfiguration -Enable $true`. See [`../../docs/365/password-reset-sspr.md`](../../docs/365/password-reset-sspr.md). | **elevated** |
