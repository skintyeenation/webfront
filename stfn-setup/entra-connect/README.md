...
| [`Verify-PasswordWriteback.ps1`](Verify-PasswordWriteback.ps1) | SSPR verify | Read-only: queries Graph `onPremisesSynchronization` for `passwordWritebackEnabled` (WAM token, silent on the DC). Expect `True` once the writeback config reports to Azure (~30 min heartbeat; `Restart-Service ADSync` to force). | on DC |
